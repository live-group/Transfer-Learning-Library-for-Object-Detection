...
from roi_data_layer.roidb import combined_roidb
from PA_ATF.roibatchLoader import roibatchLoader
...
from PA_ATF.vgg16 import vgg16
...
# log and diaplay
  ...
  parser.add_argument('--use_ins', dest='ins',
                      help='whether use ins',
                      action='store_true')
...
#    optimizer.load_state_dict(checkpoint['optimizer'])
  ...
  for epoch in range(args.start_epoch, args.max_epochs + 1):
    ...
    for step in range(iters_per_epoch):
      ...
      rois_label,DA_img_loss_cls, tgt_DA_img_loss_cls, DA_ins_loss_cls, tgt_DA_ins_loss_cls, pm_loss =\
...
#      stat(fasterRCNN, (im_data, im_info, gt_boxes, num_boxes, need_backprop, \
#                     tgt_im_data, tgt_im_info, tgt_gt_boxes, tgt_num_boxes, tgt_need_backprop))

      loss = rpn_loss_cls.mean() + rpn_loss_box.mean() \
           + RCNN_loss_cls.mean() + RCNN_loss_bbox.mean() \
             + args.lamda* (DA_img_loss_cls.mean() + DA_ins_loss_cls.mean() \
             + tgt_DA_img_loss_cls.mean() + tgt_DA_ins_loss_cls.mean()) + args.beta* pm_loss.mean()
      loss_temp += loss.item()
...
#          cross_loss = args.lamda* cross_loss.item()

          loss_pm = pm_loss.mean().item()

        print("[session %d][epoch %2d][iter %4d/%4d] loss: %.4f, lr: %.2e" \
                                % (args.session, epoch, step, iters_per_epoch, loss_temp, lr))
        print("\t\t\tfg/bg=(%d/%d), time cost: %f" % (fg_cnt, bg_cnt, end-start))
        print("\t\t\trpn_cls: %.4f, rpn_box: %.4f, rcnn_cls: %.4f, rcnn_box %.4f,\n\t\t\timg_loss: %.4f, ins_loss: %.4f, pm_loss: %.4f" \
                      % (loss_rpn_cls, loss_rpn_box, loss_rcnn_cls, loss_rcnn_box, loss_DA_img_cls, loss_DA_ins_cls, pm_loss))
        if args.use_tfboard:
          info = {
            'loss': loss_temp,
