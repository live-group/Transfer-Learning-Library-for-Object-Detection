...
from roi_data_layer.roidb import combined_roidb
from PT_MAF.roibatchLoader import roibatchLoader
...
from PT_MAF.vgg16 import vgg16
from PT_MAF.vgg16_kd import vgg16_ori
...
# config optimization
  ...
  parser.add_argument('--high', dest='high',
                      help='high',
                      default=0.7, type=float)

  parser.add_argument('--low', dest='low',
                      help='low',
                      default=0.1, type=float)
...
if __name__ == '__main__':
  ...
  if args.net == 'vgg16':
    ...
    fasterRCNN_ori = vgg16_ori(s_imdb.classes, pretrained=True, class_agnostic=args.class_agnostic)
  ...
  fasterRCNN_ori.create_architecture()

  fasterRCNN_ori.requires_grad = False
...
#        params += [{'params':[value], 'lr':lr* 10, 'weight_decay': cfg.TRAIN.WEIGHT_DECAY}]
  ...
  ori_model = '/media/4TDisk/zzl/DTOD_github_Library/github_version-master/methods/faster_rcnn/output/model/vgg16/cityscape/faster_rcnn_1_7_5931.pth'
  print('load source only model:{}'.format(ori_model))
  ori_point = torch.load(ori_model)['model']
  fasterRCNN_ori.load_state_dict(ori_point, strict=False)
  ...
  for epoch in range(args.start_epoch, args.max_epochs + 1):
    ...
    for step in range(iters_per_epoch):
      ...
      rois_label,DA_img_loss_cls,DA_ins_loss_cls,tgt_DA_img_loss_cls,tgt_DA_ins_loss_cls, rpn_prob1, cls_prob1 =\
          fasterRCNN(im_data, im_info, gt_boxes, num_boxes, need_backprop,
                     tgt_im_data, tgt_im_info, tgt_gt_boxes, tgt_num_boxes, tgt_need_backprop, args.tmp, args.high, args.low, args.alpha, args.beta, args.gamma)

      num_anchor = 12
      _, _, _, _, rpn_prob2, cls_prob2, mask = fasterRCNN_ori(im_data, im_info, gt_boxes, num_boxes, rois, args.tmp)

      rpn_prob1 = rpn_prob1.view(rpn_prob1.size(0), num_anchor* rpn_prob1.size(1), rpn_prob1.size(2) / num_anchor, rpn_prob1.size(3))
      rpn_prob2 = rpn_prob2.view(rpn_prob2.size(0), num_anchor* rpn_prob2.size(1), rpn_prob2.size(2) / num_anchor, rpn_prob2.size(3))

      pos = torch.where(rois_label > 0, torch.ones(rois_label.size()).cuda(), torch.zeros(rois_label.size()).cuda())
#      kd_loss = (cls_prob1* torch.log(cls_prob1 / cls_prob2)).mean() + (rpn_prob1* torch.log(rpn_prob1 / rpn_prob2)).mean()
      kd_loss = (1. / (pos.sum()+1))* (pos.unsqueeze(1)* cls_prob1* torch.log(cls_prob1 / cls_prob2)).sum() + (1. / (mask.sum()+1))* (mask* rpn_prob1* torch.log(rpn_prob1 / rpn_prob2)).sum()

      loss = rpn_loss_cls.mean() + rpn_loss_box.mean() \
           + RCNN_loss_cls.mean() + RCNN_loss_bbox.mean() \
             +args.lamda*(DA_img_loss_cls.mean()+ DA_ins_loss_cls.mean()\
             + tgt_DA_img_loss_cls.mean()+ tgt_DA_ins_loss_cls.mean()) + kd_loss.mean()
      loss_temp += loss.item()
...
#          loss_DA_cst = args.lamda * (DA_cst_loss + tgt_DA_cst_loss) / 2
#          loss_DA_cst = DA_cst_loss + tgt_DA_cst_loss
#          kd_loss = kd_loss.item()
        ...
        print("\t\t\trpn_cls: %.4f, rpn_box: %.4f, rcnn_cls: %.4f, rcnn_box %.4f,\n\t\t\timg_loss %.4f,ins_loss %.4f,,kl_loss %.4f" \
                      % (loss_rpn_cls, loss_rpn_box, loss_rcnn_cls, loss_rcnn_box,loss_DA_img_cls,loss_DA_ins_cls, kd_loss))
        if args.use_tfboard:
          info = {
            'loss': loss_temp,
