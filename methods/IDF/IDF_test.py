...
if __name__ == '__main__':
  ...
  # from model.faster_rcnn.vgg16_idf import vgg16
  #from model.faster_rcnn.resnet_idf import resnet
  ...
  for i in range(num_images):
      ...
      rois_label, _,_, _, _,   _,_,_,_,   _,_,_,_ = fasterRCNN(im_data, im_info, gt_boxes, num_boxes, gt_boxes, num_boxes, isSeparation)
      #rois_label, _,_, _, _,   _ = fasterRCNN(im_data, im_info, gt_boxes, num_boxes, gt_boxes, num_boxes, isSeparation)
      #print(cls_prob.shape) #1*300*9
      #print(rois.shape)     #1*300*5
      '''rois_t, cls_prob_t, bbox_pred_t, \
      rpn_loss_cls_t, rpn_loss_box_t, \
      RCNN_loss_cls_t, RCNN_loss_bbox_t, \
      rois_label_t, _,_, _, _,   _,_,_,_,   _,_,_,_ = fasterRCNN(im_data, im_info, gt_boxes, num_boxes, gt_boxes, num_boxes, isSeparation, target=True)
      
      cls_prob = torch.cat((cls_prob, cls_prob_t), 1)
      rois = torch.cat((rois, rois_t), 1)
      bbox_pred = torch.cat((bbox_pred, bbox_pred_t), 1)'''
