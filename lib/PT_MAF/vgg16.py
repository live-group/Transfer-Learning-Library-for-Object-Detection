...
from PT_MAF.faster_rcnn import _fasterRCNN
import pdb
import os
...
class vgg16(_fasterRCNN):
  ...
  def _init_modules(self):
    ...
    self.conv3 = nn.Sequential(*list(vgg.features._modules.values())[:16])
    self.conv34 = nn.Sequential(*list(vgg.features._modules.values())[16:23])
    self.conv45 = nn.Sequential(*list(vgg.features._modules.values())[23: -1])
