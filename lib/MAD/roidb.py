...
from MAD.factory import get_imdb
import PIL
import pdb
...
def rank_roidb_ratio(roidb):
    ...
    ratio_large = 4.0 # largest ratio to preserve.
...
def filter_roidb(roidb):
    ...
    while i < len(roidb):
      ...
      for j in range(len(roidb[i]['boxes'])):
        if roidb[i]['boxes'][j][0] == roidb[i]['boxes'][j][2] or roidb[i]['boxes'][j][1] == roidb[i]['boxes'][j][3]:
          empty += 1
          # print(empty)
      if len(roidb[i]['boxes']) == 0 or empty == len(roidb[i]['boxes']):
        del roidb[i]
        i -= 1
      i += 1
...
def combined_roidb(imdb_names, training=True):   #dataset name
  ...
  def get_roidb(imdb_name):
    ...
    print('dataset path:{}'.format(imdb._data_path))
    print('loaded image set file:{}'.format(imdb._data_path + '/ImageSets/Main/' + imdb._image_set + '.txt'))
